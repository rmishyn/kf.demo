...
/// Protocol to define database entities which use identifiers of `String` type
protocol DBIdentifibleByStringId: DBEntity {
    /// Entity identifier
    ...
    static func item(byId id: String, in context: NSManagedObjectContext, create:Bool) -> Self?
    ...
    ///     - id: Identifier to be used on search
    ///     - context: Context used to perform search inside it
    static func remove(byId id: String, in context: NSManagedObjectContext)
}

extension DBIdentifibleByStringId {
    
    static func item(byId id: String, in context: NSManagedObjectContext, create: Bool) -> Self? {
        let request = NSFetchRequest<Self>(entityName: entityName)
        request.predicate = NSPredicate(format: "\(Mapping.id) = %@", id)
    ...
    static func save(item incoming: [String:Any], context: NSManagedObjectContext) -> Self? {
        ...
        if let id = _incoming[Mapping.id] as? String {
